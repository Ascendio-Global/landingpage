...
type UserRole = 'student' | 'recruiter' | 'alumni' | 'admin' | 'rep' | null;

interface LandingPageProps {
    onLogin: (role: any, name: string) => void | Promise<void>;
}
...
export function LandingPage({ onLogin }: LandingPageProps) {
    ...
    const handleAuthSuccess = (role: UserRole, name: string, _uid: string) => {
        void _uid;
        onLogin(role, name);
    };
                            ...
                            {policyCards.map((card, index) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`flex items-center gap-4 shrink-0 rounded-xl p-3 text-sm font-bold text-left transition-all duration-300 ${
                                            isActive
                                                ? isDarkMode
                                    ...
                                    >
                                        <span className={`grid h-8 w-8 shrink-0 place-items-center rounded-full text-[11px] font-black transition-colors duration-300 ${
                                            isActive
...
function PolicyStackCard({ 
    ...
    return (
        <motion.article
            ...
            className={`w-full min-h-88 overflow-hidden rounded-2xl border p-7 shadow-2xl transition-all duration-300 ease-out origin-center md:p-9 ${
                index === 0 
                    ? 'bg-(--landing-accent) text-white' 
                    : isDarkMode 
